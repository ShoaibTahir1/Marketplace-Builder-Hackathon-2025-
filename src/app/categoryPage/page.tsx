...
const Page = () => {
  ...
  const handleWishlist = (carId: string) => {
  ...
  const carsToShow = showAll ? [...filteredPopularCars, ...filteredRecommendedCars] : [
    ...filteredPopularCars.slice(0, 4),
    ...filteredRecommendedCars.slice(0, 8)
  ];
      ...
      <div className="w-[80%] p-4 bg-[#F6F7F9] mx-auto">
        ...
        <div className="my-2 w-fit mx-auto sm:mx-0 sm:w-full justify-center gap-8 flex flex-col sm:flex-row sm:flex-wrap sm:justify-between">
          {carsToShow.map((car, index) => (
            <CarCard key={index} car={car} type={car._type} wishlist={wishlist} onWishlistChange={handleWishlist} />
          ))}
        </div>
...
const CarCard = ({ car, type, wishlist, onWishlistChange }: { car: any; type: string; wishlist: string[]; onWishlistChange: (carId: string) => void }) => {
  return (
    <Link href={`detailcars/${type}/${car._id}`}>
      ...
      <div className="bg-white rounded-lg w-fit p-4">
        ...
        <div className="flex justify-between">
          ...
          <button onClick={(e) => { e.preventDefault(); onWishlistChange(car._id); }} className={`text-2xl ${wishlist.includes(car._id) ? 'text-red-500' : 'text-gray-300'}`}>
            ♥
          </button>
        </div>
...
// import { client } from "@/sanity/lib/client";
// import { urlFor } from "@/sanity/lib/image";
// import Pickoff from "@/components/Pickoff";
