...
//   // Fetch recommended cars on mount
//   useEffect(() => {
...
//         {recommendedCars.slice(0, showAll ? recommendedCars.length : 8).map((r: any, index: number) => (
...
//                 </h1>
//                 <Link href={`/billing/recommendedcars/${r._id}`}>
...
const Section2 = () => {
  ...
  return (
    <div className="px-6 py-8 bg-[#F6F7F9]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6">
        {recommendedCars.slice(0, showAll ? recommendedCars.length : 8).map((r: any, index: number) => (
          <Link key={index} href={`/detailcars/RecommendedCar/${r._id}`}>
            <div className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition duration-300 h-full flex flex-col justify-between">
              {/* Car Type */}
              <h1 className="text-gray-500 font-medium">{r._type}</h1>
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                <Link href={`/billing/Recommendedcar/${r._id}`}>
                  <button className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-md transition">
                    Rent Now
                  </button>
                </Link>
